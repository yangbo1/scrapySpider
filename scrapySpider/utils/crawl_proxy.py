...
from scrapySpider.utils.thread_pool import ThreadPool
...
# 代理ip列表在缓存中的命名
PROXY_IP_NAMESPACE = 'POOL_PROXY_IPS_FUCK'
USEFUL_PROXY = 'useful_proxy'
# 缓存中代理 ip 失效时间（s）
...
import random
def get_ips(pages=3, refresh=False, count=0):
    data = cache.hgetall(name=USEFUL_PROXY)
    while not data:
        print('redis缓存数据为空')
        time.sleep(1)
        data = cache.hgetall(name=USEFUL_PROXY)
        # return crawl_quick(pages, count)
        # if random.randint(0, 9) % 2 == 0:
        #     return crawl_quick(pages)
        # else:
        #     return crawl_xici(pages)
    return data

def del_ip(ip):
    # 删除redis中无效的ip
    cache.hdel(USEFUL_PROXY, ip)
...
def sub_thread(ip_info):
    ...
    if check_ip(ip_info.get('ip')):
        return None
        # 将可用的 ip 放入缓存
        # print("ip可用，存入redis", ip_info.get('ip'))
        # cache.hset(PROXY_IP_NAMESPACE, ip_info.get("ip"), json.dumps(ip_info, ensure_ascii=False))
        # # 如果 ip 可用，则返回 ip 的信息
        # return json.dumps(ip_info)
    else:
        # 删除redis中ip
        # print("ip无效，从redis中删除", ip_info.get('ip'))
        # cache.hdel(PROXY_IP_NAMESPACE, ip_info.get("ip"))
        return None

def crawl_quick(page=3, count=0):
    """
     请求 快代理 爬取高匿代理 ip\n
    :param page:
    :return:
    """
    print(u'请求 快代理 爬取高匿代理 ip', count*10, count*10 + 10, '页')
    # assert 1 <= page <= 10, '页数有效范围为（1 - 10）'
    # validate_ips = []
    validate_ips = {}
    # for i in range(count*10, count*10 + 10):
        ...
        if response.status == 200:
            ...
            for task in executor.completed_tasks(tasks):
                ...
                if data:
                    ...
                    validate_ips[eval(data)["ip"]] = data
            # 降低爬取频率
            time.sleep(2.5)
    # 当还没有子线程返回可用的 ip 时，再次查询缓存
    if not validate_ips:
        # validate_ips = cache.lrange(name=PROXY_IP_NAMESPACE, start=0, end=100)
        validate_ips = cache.hgetall(name=PROXY_IP_NAMESPACE).values()
        # validate_ips = crawl_quick(page=0, count=count+1)
    # 设置缓存超时时间
